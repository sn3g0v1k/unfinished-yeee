...
def main(page: ft.Page):
    page.title = "Filesystem Navigation Drawer (Accordion)"
    ...
    # Настройки
    # --------------------------------
    # Корневая папка — отображение и навигация не выйдут за её пределы
    ...
    # Контент справа — покажем выбранный файл/путь
    selected_path_text = ft.Text("")
    file_preview_text = ft.Text("")
    # selected_path_text._id("selected_path")
    # file_preview_text._id("file_preview")
    selected_path_text.__uid = "selected_path"
    file_preview_text.__uid = "file_preview_text"
    # selected_path_text._id = "selected_path"
    # file_preview_text._id = "file_preview"
    content_box = ft.Container(
        ...
        content=ft.Column(
            controls=[
                ft.Text("Content Area", size=24, weight=ft.FontWeight.BOLD),
                ft.Divider(),
                ft.Text("Выбранный путь:"),
                # selected_path_text,
                # file_preview_text,
            ],
            spacing=10,
    ...
    # Собираем макет: тонкая колонка кнопки | drawer | content
    layout = ft.Row(
        controls=[
            toggle_column,
            drawer,
